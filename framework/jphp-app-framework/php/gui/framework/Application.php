...
public function __construct($configPath = null)
{
    ...
    $functionsPath = 'res://php/gui/framework/functions';

    if (Stream::exists($functionsPath . ".phb")) {
        try {
            $module = new Module("$functionsPath.phb", true);
            $module->call();
        } catch (Exception $e) {
            Logger::error("Failed to load module from '$functionsPath.phb': " . $e->getMessage());
            throw new Exception("Failed to load module from '$functionsPath.phb'", 0, $e);
        }
    } else {
        try {
            include_once "$functionsPath.php";
        } catch (Exception $e) {
            Logger::error("Failed to include functions file '$functionsPath.php': " . $e->getMessage());
            throw new Exception("Failed to include functions file '$functionsPath.php'", 0, $e);
        }
    }
    ...
    try {
        ...
        Logger::error("Unable to find the '$configPath' config: " . $e->getMessage());
        throw new Exception("Unable to find the '$configPath' config", 0, $e);
    }
}
        ...
        $onStart = function () use ($mainFormClass, $splashFormClass, $showMainForm, $handler, $after) {
            ...
            $startMain = function () use ($mainFormClass, $showMainForm, $after) {
                try {
                    ...
                    Stream::putContents(System::getProperty('spark.path') . '/bin/logs/crash.log',
