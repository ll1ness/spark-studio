...
class WindowsApplicationBuildType extends AbstractBuildType
{
    ...
    function makeExecutableFile(Project $project)
    {
        ...
        if (!$launch4j) {
            Ide::showError('Невозможно собрать исполняемый файл, не найдена утилита Launch4j');
        ...
        if ($config['oneJar']) {
            foreach (File::of($this->getBuildPath($project) . '/lib')->findFiles() as $file) {
                if (Str::startsWith($file->getName(), "sprk-compile")) {
                    $template->setJarFile('lib/' . $file->getName());
                }
            }
        }

        if (isset($config['minimizeToTray'])) {
            $template->setMinimizeToTray($config['minimizeToTray']);
        }

        if (isset($config['javaVersion'])) {
            $template->setJavaVersion($config['javaVersion']);
        }

        if (isset($config['appName'])) {
            $template->setAppName($config['appName']);
        }

        if (isset($config['appDescription'])) {
            $template->setAppDescription($config['appDescription']);
        }

        if (isset($config['appVersion'])) {
            $template->setAppVersion($config['appVersion']);
        }

        if (isset($config['debugMode'])) {
            $template->setDebugMode($config['debugMode']);
        }

        $file = $this->getLaunch4jConfigPath($project);

        $stream = new FileStream($file, 'w+');
        $template->apply($this, $stream);
        $stream->close();

        $command = "\"{$launch4j}\" \"{$file}\"";
