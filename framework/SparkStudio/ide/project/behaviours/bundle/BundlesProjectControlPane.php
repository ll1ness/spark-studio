...
class BundlesProjectControlPane extends AbstractProjectControlPane
{
                    ...
                    (new Thread(function () use ($zip, $file, $config, $code, $callback) {
                        try {
                            ...
                            uiLater(function () use ($resource) {
                                ...
                                $msg = new MessageBoxForm('Для корректного завершения установки пакета перезапустите Spark!', ['Перезапустить', 'Позже']);
    ...
    public function addBundleFile()
    {
        ...
        $dialog->extensionFilters = [['description' => 'Пакеты для Spark (*.dnbundle)', 'extensions' => ['*.dnbundle']]];
