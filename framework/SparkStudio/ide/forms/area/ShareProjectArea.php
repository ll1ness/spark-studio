...
class ShareProjectArea extends AbstractFormArea
{
    ...
    public function reUpload($silent = false)
    {
        ...
        $this->showPreloader('Загружаем проект на hub.spark.org ...');
            ...
            Ide::service()->projectArchive()->uploadArchiveAsync($res->result('id'), $file, function (ServiceResponse $response) use ($project, $silent) {
                if ($response->isSuccess()) {
                    ...
                    if (!$silent) {
                        ...
                        uiLater(function () {
                            Notifications::show('Изменения загружены', 'Измненения в проекте были успешно загружены на hub.spark.org');
                        });
                    }
                } else {
                    $this->hidePreloader();
    ...
    {
        if (!MessageBoxForm::confirm('Вы точно хотите загрузить изменения в проекте на spark.org?', $this)) {
            return;
        }
    ...
    public function doShareButtonAction()
    {
        ...
        if (!MessageBoxForm::confirm('Вы точно хотите загрузить проект на spark.org?', $this)) {
            return;
        }
        ...
        $response->on('success', function (ServiceResponse $response) use ($file, $failedCallback) {
            ...
            $response->on('success', function (ServiceResponse $response) use ($projectUid, $failedCallback) {
                Notifications::show('Проект загружен', 'Ваш проект был успешно загружен и опубликован на hub.spark.org', 'SUCCESS');
