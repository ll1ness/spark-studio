...
class Notifications
{
    ...
    static function attachException(UXTrayNotification $notify, \Exception $e) {
        $notify->on('click', function () use ($e) {
            ...
            $dialog->title = 'Ошибка';
            $dialog->headerText = 'Произошла ошибка в Spark, сообщите об этом авторам';
            $dialog->contentText = $e->getMessage();
            $dialog->setButtonTypes(['Выход из Spark', 'Продолжить']);
    ...
    {
        static::show('Приветствие', 'Добро пожаловать в социальную сеть Spark для разработчиков', 'INFORMATION');
    }
