        ...
        try {
            ...
            try {
                ...
                if (!Ide::get()->isSameVersionIgnorePatch($prVersion) && $project->getConfig()->getIdeVersionHash() > Ide::get()->getVersionHash()) {
                    $msg = new MessageBoxForm("Проект '{$project->getName()}' создан в более новой версии Spark ($prVersion), вы точно хотите его открыть?", [
            ...
            Notifications::error('Поврежденный проект', 'Проект "' . fs::nameNoExt($fileName) . '" невозможно открыть, он поврежден или создан в новой версии Spark.');
        }
