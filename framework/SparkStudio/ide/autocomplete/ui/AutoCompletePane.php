    ...
    public function getString($onlyName = false)
{
    ...
    private function makeDescriptionUi()
    {
        ...
        $ui->classes->add('sprk-autocomplete-description');
  ...
  private function makeUi()
{
    ...
    $list->classes->addAll(['hide-hor-scroll', 'sprk-console-list', 'sprk-autocomplete']);
    ...
    $list->on('action', function () use ($list) {
        ...
        if ($item && ($item->getDescription() || $item->getContent())) {
    ...
    $updateDescriptionPosition = function () {
        $this->uiDescription->x = $this->ui->x + $this->ui->width + 3;
        $this->uiDescription->y = $this->ui->y + 3;
    };
    $win->observer('x')->addListener($updateDescriptionPosition);
    $win->observer('y')->addListener($updateDescriptionPosition);
