...
class Ide extends Application
{
    ...
    {
        $launcher = System::getProperty('spark.launcher');
        ...
        if ($this->isDevelopment() && !$path->exists()) {
            $path = $this->getOwnFile('../spark-tools/');
        }
    ...
    {
        $homePath = System::getProperty('spark.path', "./");
    ...
    public function registerAll()
    {
        ...
        if (!fs::isDir($ideConfig->get('projectDirectory'))) {
            $ideConfig->set('projectDirectory', File::of(System::getProperty('user.home') . '/SparkProjects/'));
        }
    ...
    public function shutdown()
    {
        ...
        $shutdownTh->setName("SparkStudio выключение");
        $shutdownTh->start();
    ...
    protected function handleArgs($argv)
    {
        ...
        if (str::startsWith($arg, 'spark://')) {
            $arg = str::sub($arg, str::length('spark://'));
    ...
    public function restart()
    {
        ...
        $process = new Process([$javaBin, '-jar', 'Spark.jar'], IdeSystem::getOwnFile(''), $this->makeEnvironment());
        $process->start();
